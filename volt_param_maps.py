...
for wattage in wattages:
    ...
    if (platform.system() == 'Linux' or platform.system() == 'Darwin'):  # darwin = macos
        np.save(file = f'Parameter maps/a_map_{wattage}.npy', arr = a_maps[i, :, :])
    else:  # windows
        np.save(file = f'Parameter maps\\a_map_{wattage}.npy', arr = a_maps[i, :, :])

    if (platform.system() == 'Linux' or platform.system() == 'Darwin'):  # darwin = macos
        np.save(file = f'Parameter maps/b_map_{wattage}.npy', arr = b_maps[i, :, :])
    else:  # windows
        np.save(file = f'Parameter maps\\b_map_{wattage}.npy', arr = b_maps[i, :, :])

    if (platform.system() == 'Linux' or platform.system() == 'Darwin'):  # darwin = macos
        np.save(file = f'Parameter maps/c_map_{wattage}.npy', arr = c_maps[i, :, :])
    else:  # windows
        np.save(file = f'Parameter maps\\c_map_{wattage}.npy', arr = c_maps[i, :, :])
