...
def plotje_true_var_vs_watt():
    for volt in voltages:
        variancewattages_lst=[true_pixel_variance(str(volt)+"kV\\"+str(watt)+"W", pixel_x, pixel_y) for watt in wattages]
        plt.plot(wattages, variancewattages_lst, marker="o")
    plt.legend()
    plt.title("True variance vs wattage for 5 different voltages")
    plt.ylabel("variance (darkfield nog niet eraf gehaald)")
    plt.xlabel("wattage")
    plt.legend(["30kV", "45kV", "60kV", "75kV", "90kV"])
    plt.show()
...
def plotje_true_avg_vs_watt():
    for volt in voltages:
        averagewattages_lst=[true_pixel_average(str(volt)+"kV\\"+str(watt)+"W", pixel_x, pixel_y) for watt in wattages]
        plt.plot(wattages, averagewattages_lst, marker="o", label=f"{volt} kV")
    plt.title("True average pixel value vs wattages for different voltages")
    plt.xlabel("Wattages")
    plt.ylabel("Averages(geen darkfield afgetrokken)")
    plt.legend(["30kV", "45kv", "60kV", "75kV", "90kv"])
    plt.show()
